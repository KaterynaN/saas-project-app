class ProjectsController < ApplicationController
  ...
  def users
    @project_users = (@project_users + (User.where(tenant_id, is_admin: true))) - [current_user]
  ...
  def verify_tenant
    unless params[:tenant_id] == Tenant.current_tenant_id.to_s
      redirect_to :root, flash: { error: 'You are not authorized to access any organization other then your own' }
    end
  end
